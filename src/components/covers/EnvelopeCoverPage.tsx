...
export interface EnvelopeCoverPageProps {
  id?: string;
  tenant?: Tenant | null;
  companyName?: string;
...
export const EnvelopeCoverPage: React.FC<EnvelopeCoverPageProps> = ({
  id,
  tenant,
  companyName,
  companyAddress,
  ...
  const isEnvelope1 = envelopeChoice === 'ENVELOPE_1';
  const envelopeLabel = isEnvelope1 
  ...
  return (
    <div id={id} className="landscape-unified-cover single-page-paper landscape w-full max-w-[1100px] min-h-[680px] aspect-[13/8.5] bg-white text-black p-5 sm:p-7 border-4 border-black flex flex-col justify-between font-sans relative shadow-2xl print:shadow-none print:border-4 print:p-6 print:m-0 box-border mx-auto rounded-2xl">
      ...
      {/* SECTION 3: 2-COLUMN GRID (Included Folders & Expanded Red Warning Box) */}
        ...
        <div className="col-span-5">
          <div className="p-2.5 sm:p-3 border-2 border-slate-900 bg-slate-100 rounded-xl text-left space-y-1 font-sans text-xs shadow-sm h-full flex flex-col justify-center">
            <p className="font-black text-black uppercase border-b border-slate-300 pb-1 text-[10.5px] flex items-center gap-1.5">
              <span>📦</span> INCLUDED FOLDERS IN THIS ENVELOPE:
            </p>
            <ul className="text-[11px] text-slate-800 space-y-1 pt-1 font-medium">
              <li className="flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-blue-600 shrink-0" />
                <span><strong>Original Copy</strong> ({isEnvelope1 ? 'Technical & Legal' : 'Financial'})</span>
              </li>
              <li className="flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-emerald-600 shrink-0" />
                <span><strong>Copy 1</strong> (First Certified True Duplicate Copy)</span>
              </li>
              <li className="flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-purple-600 shrink-0" />
                <span><strong>Copy 2</strong> (Second Certified True Triplicate Copy)</span>
              </li>
            </ul>
          </div>
        </div>
      ...
      {/* SECTION 4: ENVELOPE IDENTIFIER BANNER (White Background with Ink Blue Text - 25% Larger & Prominent) */}
      <div className="my-1.5 relative z-10">
        <div className="py-4 sm:py-5 px-4 sm:px-6 border-4 border-blue-950 bg-white text-center rounded-2xl space-y-1 shadow-lg">
          <div className="font-mono font-black text-2xl sm:text-3xl md:text-[34px] uppercase tracking-widest text-blue-950 leading-tight">
            {envelopeLabel}
          </div>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <DocumentQrCode
            details={{
              ...
              documentName: isEnvelope1 ? 'Envelope 1 Technical Outer Cover' : 'Envelope 2 Financial Outer Cover',
...
export default EnvelopeCoverPage;
