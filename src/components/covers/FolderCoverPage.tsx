...
export interface FolderCoverPageProps {
  id?: string;
  tenant?: Tenant | null;
  companyName?: string;
  ...
  abc?: string;
  folderCopy?: 'ORIGINAL' | 'COPY_1' | 'COPY_2' | string;
  envelopeChoice?: 'ENVELOPE_1' | 'ENVELOPE_2';
  submissionDeadline?: string;
  signatoryName?: string;
  signatoryTitle?: string;
}

export const FolderCoverPage: React.FC<FolderCoverPageProps> = ({
  id,
  tenant,
  companyName,
  companyAddress,
  ...
  const copyDisplay = normalizedCopy.includes('ORIGINAL')
    ...
    : (normalizedCopy === 'COPY_1' || normalizedCopy === 'COPY 1')
      ...
      : (normalizedCopy === 'COPY_2' || normalizedCopy === 'COPY 2')
        ? 'COPY NO. 2 (TRIPLICATE)'
        : normalizedCopy.replace(/_/g, ' ');

  const envelopeTitle = envelopeChoice === 'ENVELOPE_1'
    ? 'ENVELOPE 1: ELIGIBILITY & TECHNICAL COMPONENT'
    : 'ENVELOPE 2: FINANCIAL BID PROPOSAL COMPONENT';
  ...
  return (
    <div id={id} className="landscape-unified-cover single-page-paper landscape w-full max-w-[1100px] min-h-[680px] aspect-[13/8.5] bg-white text-black p-5 sm:p-7 border-4 border-black flex flex-col justify-between font-sans relative shadow-2xl print:shadow-none print:border-4 print:p-6 print:m-0 box-border mx-auto rounded-2xl">
      ...
      {/* SECTION 3: 2-COLUMN GRID (Folder Specifications & Expanded Red Warning Box) */}
      <div className="grid grid-cols-12 gap-3 relative z-10 items-stretch mb-1.5">
        
        {/* Left Column (5 cols): Folder Designation Details */}
        <div className="col-span-5">
          <div className="p-2.5 sm:p-3 border-2 border-slate-900 bg-slate-100 rounded-xl text-left space-y-1 font-sans text-xs shadow-sm h-full flex flex-col justify-center">
            <p className="font-black text-black uppercase border-b border-slate-300 pb-1 text-[10.5px] flex items-center gap-1.5">
              <span>📁</span> FOLDER SUBMISSION DETAILS:
            </p>
            <ul className="text-[11px] text-slate-800 space-y-1 pt-1 font-medium">
              <li className="flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-blue-600 shrink-0" />
                <span><strong>Component:</strong> {envelopeChoice === 'ENVELOPE_1' ? 'Envelope 1 (Technical)' : 'Envelope 2 (Financial)'}</span>
              </li>
              <li className="flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-emerald-600 shrink-0" />
                <span><strong>Copy Designation:</strong> {copyDisplay}</span>
              </li>
              <li className="flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-purple-600 shrink-0" />
                <span><strong>Legal Standard:</strong> RA 12009 / RA 9184 PBDs</span>
              </li>
            </ul>
          </div>
        </div>
      ...
      {/* SECTION 4: FOLDER IDENTIFIER BANNER (DEDICATED EXCLUSIVELY TO THE SELECTED COPY) */}
      <div className="my-2 relative z-10">
        <div className="py-3.5 sm:py-4 px-4 sm:px-6 border-4 border-blue-950 bg-white text-center rounded-2xl space-y-1 shadow-lg">
          <div className="font-mono font-black text-2xl sm:text-3xl md:text-[32px] uppercase tracking-widest text-blue-950 leading-tight">
            {copyDisplay}
          </div>
          <div className="text-lg sm:text-xl md:text-[23px] font-sans font-black text-blue-950 uppercase tracking-wider pt-1">
            {envelopeTitle}
          </div>
        </div>
      </div>
      ...
      <div className="border-t-2 border-black pt-2 pb-0.5 flex items-center justify-between relative z-10 text-xs mt-auto">
        <div className="space-y-0.5 text-left">
          <p className="text-[9px] font-mono font-bold uppercase text-slate-500">
            CERTIFIED TRUE & AUTHENTIC BID COPY BY:
          </p>
        ...
        <div className="flex items-center gap-2.5">
          <div className="text-right text-[9px] font-mono text-slate-600 hidden sm:block leading-tight">
            <div className="font-bold text-black">{normalizedCopy} Folder Copy</div>
            <div>{projectRefNo}</div>
          </div>
          <DocumentQrCode
            details={{
              documentNumber: projectRefNo || 'PhilGEPS-2026-001',
              documentName: `${copyDisplay} — ${envelopeTitle} Cover`,
...
export default FolderCoverPage;
