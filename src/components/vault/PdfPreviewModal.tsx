...
import { X, FileText, Printer, Download, ZoomIn, ZoomOut, ExternalLink, Loader2 } from 'lucide-react';
...
export const PdfPreviewModal: React.FC<PdfPreviewModalProps> = ({ item, tenant: _tenant, onClose, pdfDataUrl, hidePrintExport: _hidePrintExport = false }) => {
  ...
  const [autoGeneratedPdf, setAutoGeneratedPdf] = useState<string | null>(null);
  const [isAutoGenerating, setIsAutoGenerating] = useState<boolean>(false);

  useEffect(() => {
    if (pdfDataUrl || item.fileDataUrl) return;

    let isMounted = true;
    setIsAutoGenerating(true);

    resolveDocumentPdfAttachment(
      {
        id: item.id,
      ...
      {
        tenant: _tenant,
        projectRefNo: item.philgepsRefNo || '',
        projectTitle: item.documentName
      }
    )
      .then((generated) => {
        if (isMounted && generated) {
          setAutoGeneratedPdf(generated);
        }
      })
      .catch((err) => {
        console.warn('Auto-generation for preview failed:', err);
      })
      .finally(() => {
        if (isMounted) setIsAutoGenerating(false);
      });

    return () => {
      isMounted = false;
    };
  }, [item, pdfDataUrl, _tenant]);

  // Use in-memory pdfDataUrl first, then item stored value, then auto-generated PDF
  const effectiveDataUrl = pdfDataUrl || item.fileDataUrl || autoGeneratedPdf;
  ...
  // When viewing PDF in browser, a blob: URL is universally supported by Chromium/Edge/Chrome/Firefox
  const viewPdfSrc = useMemo(() => {
    if (pdfBlobUrl) {
      return `${pdfBlobUrl}#toolbar=1&navpanes=0&scrollbar=1`;
    }
    if (effectiveDataUrl && !effectiveDataUrl.startsWith('data:image/')) {
      return effectiveDataUrl;
    }
    return '';
  }, [pdfBlobUrl, effectiveDataUrl]);
  ...
  const handleExportPdf = async () => {
    ...
    await generateAndDownloadThreeLayerPdf(null, templateElem, effectiveDataUrl || undefined, fileName);
  };

  return (
    <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-md flex items-center justify-center p-2 sm:p-4 overflow-y-auto">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-5xl overflow-hidden shadow-2xl animate-fadeIn my-auto max-h-[95vh] flex flex-col">
        ...
        <div className="p-4 sm:p-6 overflow-y-auto flex-1 bg-slate-950 space-y-4 text-center">
          ...
          <div style={zoomLevel !== 100 ? { transform: `scale(${zoomLevel / 100})`, transformOrigin: 'top center' } : undefined} className="transition-transform duration-200">
            ...
            <div className="space-y-3 max-w-[850px] mx-auto">
              ...
              {effectiveDataUrl ? (
                isImage ? (
                  ...
                  <div className="w-full min-h-[760px] h-[82vh] rounded-2xl overflow-hidden border-2 border-slate-800 bg-slate-900 shadow-2xl mx-auto p-1 relative flex flex-col">
                    <object
                      ...
                      className="w-full h-full min-h-[760px] border-none rounded-xl bg-white"
                    >
                      <iframe
                        src={viewPdfSrc}
                      ...
                      >
                        <div className="p-8 text-center text-slate-300 space-y-4">
                          ...
                          <a
                            href={pdfBlobUrl || effectiveDataUrl}
                            target="_blank"
                            rel="noopener noreferrer"
                ...
                <div className="w-full aspect-[13/8.5] max-w-[1150px] min-h-[760px] border border-slate-800 rounded-2xl bg-slate-900 text-slate-100 p-6 flex flex-col items-center justify-center space-y-4 shadow-2xl mx-auto">
                  <Loader2 className="w-12 h-12 text-blue-400 animate-spin" />
                  <div className="space-y-1 text-center font-mono">
                    <h4 className="text-sm font-bold text-white uppercase tracking-wider">Generating Official {item.documentName}</h4>
                    <p className="text-xs text-slate-400">Rendering high-resolution statutory PDF exhibit...</p>
                  </div>
                </div>
              ) : (
