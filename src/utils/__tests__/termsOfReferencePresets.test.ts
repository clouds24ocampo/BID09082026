...
import { TOR_PRESET_TEMPLATES } from '../../components/vault/templates/TOR';
...
describe('Statutory Terms of Reference (TOR) Presets & Procurement Law Alignment', () => {
  it('verifies DPWH Infrastructure Standard Preset matches DPWH Blue Book & RA 9184/12009 standards', () => {
    ...
    const tax = computeStatutoryTaxes(5600000, dpwh.taxType, dpwh.projectCategory, dpwh.retentionRate);
  ...
  it('verifies LGU Standard Preset conforms to RA 7160, GPPB Res. 03-2025, and FOB Destination terms', () => {
    ...
    expect(lgu.inspectionAndAcceptance[0]).toContain('Inspection and Acceptance Committee (IAC)');
    ...
    const tax = computeStatutoryTaxes(1120000, lgu.taxType, lgu.projectCategory, lgu.retentionRate);
  ...
  it('verifies Barangay Standard Preset aligns with Community-Participatory Procurement under RA 12009', () => {
    ...
    expect(brgy.backgroundRationale).toContain('Community-Participatory Procurement');
