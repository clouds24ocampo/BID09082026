  ...
  for (let index = 0; index < units.length; index++) {
    ...
    if (unit.coverElement) {
      ...
      try {
        ...
        const coverPage = pdfDoc.addPage(legalLandscape);
        coverPage.drawImage(pngImage, {
          x: 0,
          y: 0,
          width: legalLandscape[0],
          height: legalLandscape[1]
        });
        console.log(`✅ Cover Page Appended (Landscape) for: ${unit.title}`);
      ...
      for (let elemIdx = 0; elemIdx < formElements.length; elemIdx++) {
        ...
        try {
          ...
          if (canvas.height <= targetCanvasPageHeight * 1.05) {
            ...
            console.log(`✅ Single-Page Form Appended (Landscape Legal) for: ${unit.title}`);
          } else {
    ...
    if (fileSource) {
      ...
      try {
        ...
        copiedPages.forEach((copiedPage) => {
          ...
          if (isPortrait) {
            copiedPage.setRotation(degrees((rotation + 90) % 360));
          }
          console.log(`-> Page size: ${width.toFixed(0)}pt x ${height.toFixed(0)}pt (Ensured Landscape)`);
          pdfDoc.addPage(copiedPage);
        });

        console.log(`✅ All ${pageIndices.length} Uploaded PDF Pages Appended (Landscape) for: ${unit.title}`);
      } catch (err) {
